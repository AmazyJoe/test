class SearchHistoriesController < ApplicationController
  before_action :set_search_history, only: %i[ show edit update destroy ]

  # GET /search_histories or /search_histories.json
  def index
    @search_histories = SearchHistory.all
  end

  # GET /search_histories/1 or /search_histories/1.json
  def show
  end
  ...
  # POST /search_histories or /search_histories.json
  def create
    @search_history = SearchHistory.new(search_history_params)

    respond_to do |format|
      if @search_history.save
        format.html { redirect_to search_history_url(@search_history), notice: "Search history was successfully created." }
  ...
  # PATCH/PUT /search_histories/1 or /search_histories/1.json
  def update
    respond_to do |format|
      if @search_history.update(search_history_params)
        format.html { redirect_to search_history_url(@search_history), notice: "Search history was successfully updated." }
  ...
  # DELETE /search_histories/1 or /search_histories/1.json
  def destroy
